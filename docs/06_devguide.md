...
| **7주** | 빌드 검증·보안 점검·KIS IP 확인 | 반응형 마무리·최종 QA·발표 슬라이드 제작 | 🔄 진행 중 |
| **8주** | 환경변수 정리·마무리 | 로컬 환경 최종 점검·데모 시나리오·발표 리허설 | ⬜ 예정 |
...
[ ] 데모용 샘플 데이터 준비 (6개월 매매 내러티브 스토리 반영)
[ ] 스크린샷 종목 등록 데모용 잔고 캡처 이미지 준비
[ ] 발표 리허설 (슬라이드 탭 ↔ 앱 탭 전환 순서)

[x] 관리자 이메일 화이트리스트 제거 — 로그인 계정이면 누구나 접근 가능하도록 변경 (하드코딩 제거)
[x] 대시보드 보유 종목 카드 탭 → 종목별 비중 바 차트 BottomSheet 팝업 (BenchmarkSheet 스타일 통일)
[x] 발표 슬라이드 (`/slides`) — 단일 페이지 풀스크린 구현 완료
  - 10개 슬라이드: 커버 / 문제 / 개발목표+범위 / 경쟁사비교 / 기술스택 / 핵심기능 / AI차별점 / 개발에피소드 / DEMO큐카드 / 클로징
  - 키보드(← →/Space) 네비게이션, 라이트/다크 모드 토글, PDF 출력 버튼
  - 경쟁사 비교 슬라이드(02-2): 더리치·도미노·증권사 앱 vs 버텨일지 ✓/✗/△ 표
  - 슬라이드 전체 폰트 업스케일 (10→12px, 11→13px, 12→14px, 13→15px, 26→30px 등)
