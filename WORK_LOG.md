...
- Prisma 스키마 작성 + Supabase 마이그레이션 완료
- NextAuth v5 Credentials + Google 설정 완료
- middleware.ts 작성 완료

### 🔄 진행 중
- Kakao 로그인 콜백 URL 설정 중

### ❌ 이슈 / 블로커
- Kakao 개발자 앱 등록 시 redirect URI 설정 방법 확인 필요

### 📌 다음 작업
- components/ui/ 공통 컴포넌트 생성 시작
- Button, Card, Tag, BottomSheet 우선

### 🔗 API 완성 목록 (B에게 공유)
- POST /api/auth/register — 완성
- NextAuth 세션 — 완성, `session.user.id` 사용 가능
...
- components/ui/ Button, Card 확인

### 🔄 진행 중
- app/onboarding/page.tsx 작업 중 (60%)
- 종목 검색 자동완성 컴포넌트 구현 중

### ❌ 이슈 / 블로커
- /api/market/search 아직 미완성 → mock data로 진행 중
  (A한테 완성되면 알려달라고 요청함)

### 📌 다음 작업
- 온보딩 완료 후 대시보드 페이지 시작
- ui-mockup.jsx Dashboard 컴포넌트 참고
