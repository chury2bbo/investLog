      ...
      <div className={`flex gap-1 p-1 rounded-2xl bg-(--color-g100) dark:bg-(--color-border) ${className}`}>
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-sm transition-all cursor-pointer active:scale-95 active:opacity-70 ${
              active === tab.key
    ...
    <div className={`flex gap-1.5 overflow-x-auto scrollbar-hide ${className}`}>
      {tabs.map((tab) => (
        <button
          key={tab.key}
          onClick={() => onChange(tab.key)}
          className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all cursor-pointer active:scale-95 active:opacity-70 ${
            active === tab.key
