...
export const REPORT_SYSTEM = `당신은 주식 애널리스트입니다. 개인 투자자용 종목 분석 리포트를 작성합니다.
적정 매수가·매도가는 반드시 현재가 기준으로 현실적 범위 내에서 산출하세요.
한국어, JSON 형식으로만 응답하세요.`;

export function buildReportUserPrompt(stockName: string, ticker: string, priceText: string) {
  return `주식 종목 "${stockName}"(${ticker}, ${priceText})에 대해 투자 분석을 해줘.
